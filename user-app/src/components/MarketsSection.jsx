      ...
      <div className="-mx-4 px-4 overflow-x-auto no-scrollbar flex items-stretch gap-3 sm:gap-4 pb-2">
        ...
        {!loading && mainBranches.map((m) => (
          <button
            ...
            className="flex-shrink-0 w-56 sm:w-64 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-soft hover:shadow-md transition-transform hover:-translate-y-1 cursor-pointer text-left"
          >
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-800 flex items-center justify-center text-xl font-black text-brand-600">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                  ...
                  <span className="truncate">{m.branchName || t('primary_store', 'Main Branch')}</span>
                </div>
              </div>
            </div>
            <div className="mt-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <StarRating value={m.rating ?? null} size={14} />
                <span className="text-xs text-gray-400">{m.reviewsCount ? `(${m.reviewsCount})` : ''}</span>
              </div>
