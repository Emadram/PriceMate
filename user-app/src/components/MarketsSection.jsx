    ...
    <section className="space-y-4 animate-in slide-in-from-bottom-12 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 px-2">
        {loading && [...Array(4)].map((_, i) => (
          <div key={i} className="bg-gray-100 dark:bg-gray-800 rounded-2xl p-4 animate-pulse min-h-24" />
        ))}

        {!loading && mainBranches.length === 0 && (
          <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-4 shadow-soft">
            <p className="text-sm text-gray-500">{t('no_markets', 'No markets available')}</p>
          </div>
        )}

        {!loading && mainBranches.map((m) => {
          const availability = getStoreAvailability(m);
          return (
          <button
            key={m.$id}
            onClick={() => navigate(`/supermarket/${m.$id}`)}
            className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-3 sm:p-4 shadow-soft hover:shadow-md transition-transform hover:-translate-y-1 cursor-pointer text-left min-w-0"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 min-w-0">
              <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-800 flex items-center justify-center text-lg font-black text-brand-600 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                {availability.todayHours && availability.todayHours !== 'closed' && (
                  <div className="text-[10px] text-gray-400 mt-0.5 truncate">{availability.todayHours}</div>
                )}
              </div>
            </div>
            <div className="mt-3 flex items-center justify-between min-w-0">
              ...
              <div className={`text-xs font-bold px-2 py-1 rounded-xl ${availability.isOpen ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400'}`}>
                {availability.isOpen ? t('open', 'Open') : t('closed', 'Closed')}
              </div>
            </div>
          </button>
        );})}
